...
<div class="card">
    <h5 class="card-header">Editar etiqueta de publicación</h5>
    <div class="card-body">
      <form method="post" action="{{route('post-tag.update',$postTag->id)}}">
        ...
        <div class="form-group">
          ...
          <select name="status" class="form-control">
            <option value="active" {{(($postTag->status=='active') ? 'selected' : '')}}>Activa</option>
            <option value="inactive" {{(($postTag->status=='inactive') ? 'selected' : '')}}>Inactiva</option>
          </select>
          @error('status')
          <span class="text-danger">{{$message}}</span>
          @enderror
